...
def T2():
    ...
    A_perp = -2.62e6 # [Hz] Hyperfine coupling constant in ground states, perpendicular to [1, 1, 1] axis.
    A_parallel = -2.16e6 # [Hz] hyperfine coupling constant on the z-axis
    C_perp = -40e6 # [Hz] Hyperfine coupling constant in excited states, perpendicular to [1, 1, 1] axis.
    C_parallel = -40e6 # [Hz] hyperfine coupling constant on the z-axis
    ...
    H_zfs = tensor( (2*np.pi * (Dzfs_g + Dzfs_e) * sz*sz) , In )
    H_zee_e = tensor( (2*2*np.pi*omega_e*sz), In )
    H_zee_N = tensor( Ie, (2*2*np.pi*omega_N14*Iz) )
    H_N = tensor( Ie, (2*2*np.pi*Q*Iz*Iz) )
    HI_e = 2*np.pi* (C_parallel*tensor(sz,Iz) + C_perp*(tensor(sx,Ix) + tensor(sy,Iy)))
    HI_g = 2*np.pi* (A_parallel*tensor(sz,Iz) + A_perp*(tensor(sx,Ix) + tensor(sy,Iy)))
    H_total = H_zfs + H_zee_e + H_zee_N + H_N + HI_e + HI_g
    H = H_total.ptrace(0)
...
# value for decay rate/pulse
# nuclei level for dephasing
# add C same as nuclei


# T2 relaxation time for electron triplet system with nulceus
# to be updated